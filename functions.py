...
def file_upload_brigadas(path):
    """
    En esta función se garantiza el cargue y casteo de la información 
    ...
    df['inc_barrenador'] = pd.to_numeric(df['inc_ barrenador'], downcast='float')
    df.drop('inc_ barrenador', axis = 1, inplace=True)
