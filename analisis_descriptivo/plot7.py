...
identificadores = lsp[['etapa_fenologica','variedad']]
...
dd=pd.melt(iea,id_vars=['variedad'],value_vars=[*nombres.values()],var_name='Enfermedad')
dd1=pd.melt(iea,id_vars=['etapa_fenologica'],value_vars=[*nombres.values()],var_name='Enfermedad')
######## Se calcula la paleta de colores #########
l = [*iea.columns][0:len([*iea.columns])-2] # Se restan los ultimos.
lcolores = [colores_arroz[x] for x in l]
paleta = {[*nombres.values()][0:len([*iea.columns])-2][i]: lcolores[i] for i in range(len([*nombres.values()][0:len([*iea.columns])-2]))} # Paleta de colores para grafica

############ Se realizan las graficas ############
sns.set(style="white")
fig, axes = plt.subplots(1, 2, sharex=True, figsize=(11.7,8.27))
axes[1] = sns.lineplot(x='etapa_fenologica',y='value',data=dd1,hue='Enfermedad',palette=paleta)
plt.xticks(rotation = 90)
sns.despine()
sns.despine()
axes[0] = sns.lineplot(x='variedad',y='value',data=dd,hue='Enfermedad',palette=paleta)
sns.despine()
plt.xticks(rotation = 90)
plt.ylabel('INCIDENCIA')
plt.savefig("analisis_descriptivo\\plot7.png")
